...
function [muap ,cmu_xx, cmu_yy,rmu]=channel64_MUAP(nf,FD,cmu_x,cmu_y,dist_z,xmu,ymu,skinfat,mScheme,interE)
...
cmu_xx = cmu_x; %这是调整后的MU x坐标
cmu_yy = cmu_y;%这是调整后的MU y坐标
hold on
HH=plot(cx,cy,'r');
set(HH,'LineWidth',2);
if mScheme ==2
...
for j=9:16
    for i=1:nf
        sfap=SFAP_tri_improve(cmu_xp(i)-3*IED,cmu_yp(i),dist_z+endplate(i)+(j-9)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        len(i)=length(sfap);
    end
    sfap=zeros(nf,max(len));
    for i=1:nf
        clear temp;
        temp=SFAP_tri_improve(cmu_xp(i)-3*IED,cmu_yp(i),dist_z+endplate(i)+(j-9)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        sfap(i,1:length(temp))=temp;
    end
    %一个MU的MUAP
    muap(j,:)=sum(sfap);
...
for j=17:24
    for i=1:nf
        sfap=SFAP_tri_improve(cmu_xp(i)-2*IED,cmu_yp(i),dist_z+endplate(i)+(j-17)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        len(i)=length(sfap);
    end
    sfap=zeros(nf,max(len));
    for i=1:nf
        clear temp;
        temp=SFAP_tri_improve(cmu_xp(i)-2*IED,cmu_yp(i),dist_z+endplate(i)+(j-17)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        sfap(i,1:length(temp))=temp;
    end
    %一个MU的MUAP
    muap(j,:)=sum(sfap);
...
for j=25:32
    for i=1:nf
        sfap=SFAP_tri_improve(cmu_xp(i)-IED,cmu_yp(i),dist_z+endplate(i)+(j-25)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        len(i)=length(sfap);
    end
    sfap=zeros(nf,max(len));
    for i=1:nf
        clear temp;
        temp=SFAP_tri_improve(cmu_xp(i)-IED,cmu_yp(i),dist_z+endplate(i)+(j-25)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        sfap(i,1:length(temp))=temp;
    end
    %一个MU的MUAP
    muap(j,:)=sum(sfap);
...
for j=33:40
    for i=1:nf
        sfap=SFAP_tri_improve(cmu_xp(i),cmu_yp(i),dist_z+endplate(i)+(j-33)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        len(i)=length(sfap);
    end
    sfap=zeros(nf,max(len));
    for i=1:nf
        clear temp;
        temp=SFAP_tri_improve(cmu_xp(i),cmu_yp(i),dist_z+endplate(i)+(j-33)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        sfap(i,1:length(temp))=temp;
    end
    %一个MU的MUAP
    muap(j,:)=sum(sfap);
...
for j=41:48
    for i=1:nf
        sfap=SFAP_tri_improve(cmu_xp(i)+IED,cmu_yp(i),dist_z+endplate(i)+(j-41)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        len(i)=length(sfap);
    end
    sfap=zeros(nf,max(len));
    for i=1:nf
        clear temp;
        temp=SFAP_tri_improve(cmu_xp(i)+IED,cmu_yp(i),dist_z+endplate(i)+(j-41)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        sfap(i,1:length(temp))=temp;
    end
    %一个MU的MUAP
    muap(j,:)=sum(sfap);
...
for j=49:56
    for i=1:nf
        sfap=SFAP_tri_improve(cmu_xp(i)+2*IED,cmu_yp(i),dist_z+endplate(i)+(j-49)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        len(i)=length(sfap);
    end
    sfap=zeros(nf,max(len));
    for i=1:nf
        clear temp;
        temp=SFAP_tri_improve(cmu_xp(i)+2*IED,cmu_yp(i),dist_z+endplate(i)+(j-49)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        sfap(i,1:length(temp))=temp;
    end
    %一个MU的MUAP
    muap(j,:)=sum(sfap);
...
for j=57:64
    for i=1:nf
        sfap=SFAP_tri_improve(cmu_xp(i)+3*IED,cmu_yp(i),dist_z+endplate(i)+(j-57)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        len(i)=length(sfap);
    end
    sfap=zeros(nf,max(len));
    for i=1:nf
        clear temp;
        temp=SFAP_tri_improve(cmu_xp(i)+3*IED,cmu_yp(i),dist_z+endplate(i)+(j-57)*IED,0.303,vel(i),len_fiber(i),fiberL(i),fiberR(i));
        sfap(i,1:length(temp))=temp;
    end
    %一个MU的MUAP
    muap(j,:)=sum(sfap);
